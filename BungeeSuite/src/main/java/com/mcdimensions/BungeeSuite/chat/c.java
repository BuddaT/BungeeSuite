...
public class c extends Command {
	...
	public void execute(CommandSender arg0, String[] arg1) {
		if(arg1.length==0 || arg1[0].equalsIgnoreCase("help")){
			...
			arg0.sendMessage(ChatColor.RED+"/"+plugin.c+" kick (player)"+ ChatColor.GOLD+"- Kicks player from channel");
			arg0.sendMessage(ChatColor.RED+"/"+plugin.c+" format"+ ChatColor.GOLD+"- Displays channel format");
			arg0.sendMessage(ChatColor.RED+"/"+plugin.c+" editformat (format)"+ ChatColor.GOLD+"- edits channel format");
			arg0.sendMessage(ChatColor.RED+"/"+plugin.c+" rename (name)"+ ChatColor.GOLD+"- Renames the channel");
			arg0.sendMessage(ChatColor.RED+"/"+plugin.c+" setowner (player)"+ ChatColor.GOLD+"- Changes the owner of the channel");
			arg0.sendMessage(ChatColor.RED+"/"+plugin.c+" members *(channel) "+ ChatColor.GOLD+"- Displays channel members");
			return;
		}
		...
		if(arg1[0].equalsIgnoreCase("kick")){
			if(arg1.length<2){
				arg0.sendMessage(ChatColor.RED+"/"+plugin.c+" kick (player)");
				return;
			}
		...
		if(arg1[0].equalsIgnoreCase("editformat")){
			if(arg1.length<2){
				arg0.sendMessage(ChatColor.RED+"/"+plugin.c+" editformat (format)");
				return;
			}
			String format ="";
		...
		if(arg1[0].equalsIgnoreCase("rename")){
			if(arg1.length<2){
				arg0.sendMessage(ChatColor.RED+"/"+plugin.c+" rename (name)");
				return;
			}
			String name = arg1[1];
			cc.rename(name);
		...
		if(arg1[0].equalsIgnoreCase("setowner")){
			if(arg1.length<2){
				arg0.sendMessage(ChatColor.RED+"/"+plugin.c+" setowner (name)");
				return;
			}
			String name = arg1[1];
