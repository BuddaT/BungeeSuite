...
public class ChannelCommand extends Command {
	...
	public void execute(CommandSender arg0, String[] arg1) {
		if (arg1.length == 0 || arg1[0].equalsIgnoreCase("help")) {
			...
			arg0.sendMessage(ChatColor.RED + "/" + plugin.channel + " kick (player)" + ChatColor.GOLD
					+ "- Kicks player from channel");
			arg0.sendMessage(ChatColor.RED + "/" + plugin.channel + " format" + ChatColor.GOLD + "- Displays channel format");
			arg0.sendMessage(ChatColor.RED + "/" + plugin.channel + " editformat (format)" + ChatColor.GOLD
					+ "- edits channel format");
			arg0.sendMessage(ChatColor.RED + "/" + plugin.channel + " rename (name)" + ChatColor.GOLD
					+ "- Renames the channel");
			arg0.sendMessage(ChatColor.RED + "/" + plugin.channel + " setowner (player)" + ChatColor.GOLD
					+ "- Changes the owner of the channel");
			arg0.sendMessage(ChatColor.RED + "/" + plugin.channel + " members *(channel) " + ChatColor.GOLD
		...
		if (!(arg0.hasPermission("BungeeSuite.admin") || arg0.getName()
				.equalsIgnoreCase(cc.getOwner()))) {
		...
		if (arg1[0].equalsIgnoreCase("setowner")) {
			if (arg1.length < 2) {
				arg0.sendMessage(ChatColor.RED + "/" + plugin.channel
						+ " setowner (name)");
				return;
			}
			String name = arg1[1];
		...
		if (arg1[0].equalsIgnoreCase("members")) {
			String playerList = ChatColor.DARK_AQUA + "Members: "
					+ ChatColor.WHITE;
		...
		arg0.sendMessage(ChatColor.RED + "/" + plugin.channel + " kick (player)" + ChatColor.GOLD
				+ "- Kicks player from channel");
		arg0.sendMessage(ChatColor.RED + "/" + plugin.channel + " format"
